...
```sql
user_id, name, birth_date, gender, age_group
occupation, marital_status  -- 신규 추가
```
...
1. 김지은 (20대 여성, 2004-03-15) - 대학생, 미혼
2. 박민수 (30대 남성, 1989-07-22) - 직장인, 기혼
3. 이영희 (40대 여성, 1979-11-08) - 직장인, 기혼

※ 선호 카테고리는 구매이력으로 실시간 분석됨
```
...
**① 반복 구매 상품 (최고 우선순위)** ⬆️ 강화
- 기본 점수: **100점** (기존 60점 → 100점)
- 구매 횟수 보너스: 구매 횟수 × **10점** (기존 5점 → 10점)
- 최근 구매 보너스:
  - 최근 1개월 내 반복 구매: +**20점**
  - 최근 2개월 내 반복 구매: +**10점**
- 예시: 3회 구매, 최근 1개월 내 → 100 + (3 × 10) + 20 = **150점**

**② 최근 구매한 상품과 같은 카테고리** ⬆️ 강화
- 기본 점수: **60점** (기존 40점 → 60점)
- 최근 구매 보너스: 최근 1개월 내 같은 카테고리 구매 시 +**30점** (기존 20점 → 30점)
- 예시: 최근 구매한 카테고리 상품 → 60 + 30 = **90점**

**③ 최근 구매한 상품과 유사한 상품 (같은 카테고리, 미구매)** ⬆️ 강화
- 기본 점수: **50점** (기존 30점 → 50점)
- 최근 구매일 보너스:
  - 최근 1개월 내 구매한 카테고리: +**25점** (기존 15점 → 25점)
  - 최근 2개월 내 구매한 카테고리: +**15점** (기존 10점 → 15점)
- 예시: 최근 1개월 내 구매한 카테고리의 미구매 상품 → 50 + 25 = **75점**

**④ 미구매 상품 중 선호 카테고리** ⬆️ 강화
- 기본 점수: **30점** (기존 20점 → 30점)
...
**🔄 구매이력이 있는 경우 (차별화 강화):**
```
최종 점수 = 구매이력 점수(80%) + 프로필 점수(보조) + 인기도 점수(보조)
```

**가중치 상세:**

- **구매이력 점수**: **80% 가중치** (기존 50% → 80%로 증가) ⬆️ 강화
  - 위에서 계산한 `calculatePurchaseHistoryScore` 결과 × 0.8
  - 구매이력이 있으면 구매이력 점수가 지배적으로 작용하여 유저별 차별화 강화
  
- **프로필 점수**: 보조적 가중치 (기존 30% → 보조로 감소) ⬇️ 보조화
  - 연령대 매칭: **10점** (기존 30점 → 10점)
  - 성별 매칭: **8점** (기존 20점 → 8점)
  - 카테고리 보너스: **5점** (기존 15점 → 5점, 20% 반영)
  
- **인기도 점수**: 보조적 가중치 (기존 10% → 보조로 감소) ⬇️ 보조화
  - 리뷰 수 기반: `min((reviews / 2000) × 5, 5)` (기존 10점 → 5점)

**🔄 구매이력이 없는 경우 (신규 유저):**
```
최종 점수 = 프로필 점수(주요) + 인기도 점수(보조)
```
...
- **인기도 점수**: 보조 가중치
  - 리뷰 수 기반: `min((reviews / 2000) × 15, 15)`
...
**예시 1**: 김지은(20대 여성) - 삼각김밥 모음 (반복 구매 3회, 최근 1개월 내)
```
구매이력 점수: 100 + (3 × 10) + 20 = 150점
→ 통합 점수: 150 × 0.8 = 120점 ⬆️ 강화
프로필 점수: 10 (연령대) + 8 (성별) + 5 (카테고리) = 23점 ⬇️ 보조화
인기도 점수: min((8234 / 2000) × 5, 5) = 5점 ⬇️ 보조화
최종 점수: 120 + 23 + 5 = 148점
```

**예시 2**: 박민수(30대 남성) - 제육볶음 밀키트 (반복 구매 4회, 최근 7일 전)
```
구매이력 점수: 100 + (4 × 10) + 20 = 160점
→ 통합 점수: 160 × 0.8 = 128점 ⬆️ 강화
프로필 점수: 10 (연령대) + 8 (성별) + 5 (카테고리) = 23점 ⬇️ 보조화
인기도 점수: min((7234 / 2000) × 5, 5) = 5점 ⬇️ 보조화
최종 점수: 128 + 23 + 5 = 156점
```

**예시 3**: 이영희(40대 여성) - 순두부찌개 밀키트 (반복 구매 5회, 최근 8일 전)
```
구매이력 점수: 100 + (5 × 10) + 20 = 170점
→ 통합 점수: 170 × 0.8 = 136점 ⬆️ 강화
프로필 점수: 10 (연령대) + 8 (성별) + 5 (카테고리) = 23점 ⬇️ 보조화
인기도 점수: min((5821 / 2000) × 5, 5) = 5점 ⬇️ 보조화
최종 점수: 136 + 23 + 5 = 164점
```

**💡 차별화 효과:**
- 구매이력 점수가 80% 가중치로 지배적이 되어, 각 유저의 구매 패턴에 따라 완전히 다른 상품이 추천됨
- 유저 1 (김지은): 간편식 구매이력 → 간편식/밀키트 우선 추천
- 유저 2 (박민수): 밀키트/해산물 구매이력 → 밀키트/해산물 우선 추천
- 유저 3 (이영희): 건강식/채소 구매이력 → 건강식/채소 우선 추천
...
2. 통합 점수 계산 (모든 상품)
   - 구매이력 점수 (80%) ⬆️ 강화
   - 프로필 점수 (보조, 구매이력 있으면 낮게) ⬇️ 보조화
   - 인기도 점수 (보조, 구매이력 있으면 낮게) ⬇️ 보조화
   ↓
3. 상위 30개 상품 선정
   ↓
4. GPT-4o-mini에게 전달
...
- **반복 구매 상품**: 100 + (구매횟수 × 10점) + 최근 구매 보너스 ⬆️ 강화
- **최근 구매 카테고리**: 60점 + (1개월 내 +30점) ⬆️ 강화
- **최근 카테고리의 미구매 상품**: 50점 + (1개월 +25, 2개월 +15) ⬆️ 강화
- **선호 카테고리 미구매 상품**: 30점 ⬆️ 강화
...
**Version**: 0.6.0 (Prototype - 직업/혼인여부 필드 추가 및 선호도 실시간 분석 시스템)
